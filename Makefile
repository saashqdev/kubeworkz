# Copyright 2021 The KubeWorkz Authors. All rights reserved.
# Use of this source code is governed by a Apache license
# that can be found in the LICENSE file.

IMG ?= hub.c.163.com/kubeworkz/kube:v0.0.1
MULTI_ARCH ?= true
...
build-kube: #generate fmt vet
ifeq ($(MULTI_ARCH),true)
	CGO_ENABLED=0 GOOS=linux GO111MODULE=on go build -mod=vendor -a -o kube cmd/kube/main.go
else
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 GO111MODULE=on go build -mod=vendor -a -o kube cmd/kube/main.go
endif
...
docker-build-kube: vendor
	MULTI_ARCH=false
	docker build -f ./build/kube/Dockerfile -t ${IMG} .

docker-build-kube-multi-arch: vendor
	MULTI_ARCH=true
	docker buildx build -f ./build/kube/Dockerfile -t ${IMG} --platform=linux/arm,linux/arm64,linux/amd64 . --push
