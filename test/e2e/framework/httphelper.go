...
func NewHttpHelper() *HttpHelper {
	hostPath := viper.GetString("kubeworkz.hostPath")
	admin := viper.GetString("kubeworkz.multiuser.admin")
	tenantAdmin := viper.GetString("kubeworkz.multiuser.tenantAdmin")
	projectAdmin := viper.GetString("kubeworkz.multiuser.projectAdmin")
	user := viper.GetString("kubeworkz.multiuser.user")
	adminAk := viper.GetString("kubeworkz.multiuserAkSk.adminAk")
	tenantAdminAk := viper.GetString("kubeworkz.multiuserAkSk.tenantAdminAk")
	projectAdminAk := viper.GetString("kubeworkz.multiuserAkSk.projectAdminAk")
	userAk := viper.GetString("kubeworkz.multiuserAkSk.userAk")
	adminSk := viper.GetString("kubeworkz.multiuserAkSk.adminSk")
	tenantAdminSk := viper.GetString("kubeworkz.multiuserAkSk.tenantAdminSk")
	projectAdminSk := viper.GetString("kubeworkz.multiuserAkSk.projectAdminSk")
	userSk := viper.GetString("kubeworkz.multiuserAkSk.userSk")
...
func (h *HttpHelper) GetToken(user *AuthUser) error {
	req := h.Request("GET", fmt.Sprintf("%s/api/v1/kube/key/token?accessKey=%s&secretKey=%s", h.HostPath, user.Ak, user.Sk), "", nil)
	resp, err := h.Client.Do(&req)
	if err != nil {
...
func (h *HttpHelper) FormatUrl(url string) string {
	return fmt.Sprintf("%s%s%s", h.HostPath, "/api/v1/kube", url)
}
