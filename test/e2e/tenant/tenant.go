...
var _ = ginkgo.Describe("Test Tenant and Project", func() {
	...
	ginkgo.Context("tenant and project", func() {
		...
		ginkgo.It("create tenant", func() {
			...
			tenantJson := "{\"apiVersion\":\"tenant.kubeworkz.io/v1\",\"kind\":\"Tenant\",\"metadata\":{\"name\":\"" + tenantName + "\"},\"spec\":{\"displayName\":\"my-tenant\",\"description\":\"my-tenant\"}}"
			url := "/proxy/clusters/pivot-cluster/apis/tenant.kubeworkz.io/v1/tenants"
		...
		ginkgo.It("create project without tenant", func() {
			projectJson := "{\"apiVersion\":\"tenant.kubeworkz.io/v1\",\"kind\":\"Project\",\"metadata\":{\"name\":\"project-sample\"},\"spec\":{\"description\":\"my-project\",\"displayName\":\"my-project\"}}"
			url := "/proxy/clusters/pivot-cluster/apis/tenant.kubeworkz.io/v1/projects"
			...
			framework.ExpectEqual("can not find .metadata.labels.kubeworkz.io/tenant label", string(statusErr.Reason))
		})

		ginkgo.It("create project within tenant", func() {
			projectJson := "{\"apiVersion\":\"tenant.kubeworkz.io/v1\",\"kind\":\"Project\",\"metadata\":{\"labels\":{\"kubeworkz.io/tenant\":\"" + tenantName + "\"},\"name\":\"" + projectName + "\"},\"spec\":{\"description\":\"my-project\",\"displayName\":\"my-project\"}}"
			url := "/proxy/clusters/pivot-cluster/apis/tenant.kubeworkz.io/v1/projects"
			req := f.HttpHelper.Post(f.HttpHelper.FormatUrl(url), projectJson, nil)
			_, err := f.HttpHelper.Client.Do(&req)
			framework.ExpectNoError(err)
		...
		ginkgo.It("delete project before delete .spec.namespace", func() {
			url := "/proxy/clusters/pivot-cluster/apis/tenant.kubeworkz.io/v1/projects/" + projectName
		...
		ginkgo.It("delete tenant before delete .spec.namespace", func() {
			url := "/proxy/clusters/pivot-cluster/apis/tenant.kubeworkz.io/v1/tenants/" + tenantName
			req := f.HttpHelper.Delete(f.HttpHelper.FormatUrl(url))
			resp, err := f.HttpHelper.Client.Do(&req)
			framework.ExpectNoError(err)
			defer resp.Body.Close()
