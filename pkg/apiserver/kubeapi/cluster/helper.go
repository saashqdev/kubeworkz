...
import (
	...
	"github.com/saashqdev/kubeworkz/pkg/utils/meta"
	"github.com/saashqdev/kubeworkz/pkg/utils/strproc"
)
...
func listCubeResourceQuota(ctx context.Context, cli mgrclient.Client, tenants []string, tenantsCr []tenantv1.Tenant, clusters []string) ([]kubeResourceQuotaData, error) {
	...
	res := make([]kubeResourceQuotaData, 0, len(tenants)*len(clusters))
	for _, tenant := range tenantsCr {
		for _, cluster := range clusters {
			v := kubeResourceQuotaData{
...
func sortCubeResourceQuotas(qs []kubeResourceQuotaData) []kubeResourceQuotaData {
	...
	res := []kubeResourceQuotaData{}
	bothUnsetted := []kubeResourceQuotaData{}
	oneUnsetted := []kubeResourceQuotaData{}
