...
var _ = Describe("User", func() {
	...
	It("create user", func() {
		...
		// create user by api
		router := gin.New()
		router.POST("/api/v1/kube/user", user.CreateUser)
		w := performRequest(router, http.MethodPost, "/api/v1/kube/user", userBody)
	...
	It("list user", func() {
		...
		router.GET("/api/v1/kube/user", user.ListUsers)
		...
		w := performRequest(router, http.MethodGet, "/api/v1/kube/user", []byte(""), auth)
	...
	It("update user", func() {
		...
		// update user by api
		router := gin.New()
		router.PUT("/api/v1/kube/user/:username", user.UpdateUser)
		w := performRequest(router, http.MethodPut, "/api/v1/kube/user/admin", userBody)
		Expect(w.Code).To(Equal(http.StatusOK))
