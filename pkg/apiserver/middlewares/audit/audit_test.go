...
func TestGetEventName(t *testing.T) {
	...
	// check post method
	router1 := gin.New()
	router1.POST("/api/v1/kube/proxy/clusters/:cluster/api/v1/namespaces/:namespace/services", func(c *gin.Context) {
		e = h.handleProxyApi(ctx, c, *e)
		return
	})
	_ = performRequest(router1, http.MethodPost, "/api/v1/kube/proxy/clusters/pivot-cluster/api/v1/namespaces/dev/services", []byte(""))
	...
	// check put method
	router2 := gin.New()
	router2.PUT("/api/v1/kube/proxy/clusters/:cluster/api/v1/namespaces/:namespace/secrets/:name", func(c *gin.Context) {
		e = h.handleProxyApi(ctx, c, *e)
		return
	})
	_ = performRequest(router2, http.MethodPut, "/api/v1/kube/proxy/clusters/pivot-cluster/api/v1/namespaces/dev/secrets/secretA", []byte(""))
