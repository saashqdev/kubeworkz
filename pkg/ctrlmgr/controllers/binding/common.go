...
// {user}-in-kubeworkz-tenant-{tenantName}/kubeworkz-tenant-{tenantName}
// {user}-in-kubeworkz-project-{projectName}/kubeworkz-project-{projectName}
//
...
func parseUserInRoleBinding(name, namespace string) (user string, tenant string, project string, err error) {
	tenantInfos := strings.Split(name, "-in-kubeworkz-tenant-")
	projectInfos := strings.Split(name, "-in-kubeworkz-project-")
