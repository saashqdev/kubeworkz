...
var _ = ginkgo.Describe("Filter", func() {
	...
	ginkgo.BeforeEach(func() {
		...
		for i := 0; i < 20; i++ {
			l2 := v1.Cluster{}
			l2.Kind = "Cluster"
			l2.APIVersion = "cluster.kubeworkz.io/v1"
			if i <= 10 {
			...
			annotations["kubeworkz.test.io/index"] = strconv.Itoa(i)
			annotations["kubeworkz.test.io/app"] = "a-name" + strconv.Itoa(i)
	...
	ginkgo.It("TestArrayExactMatch", func() {
		// create condition
		exact := make(map[string]sets.Set[string])
		exact["metadata.annotations.kubeworkz.test.io/app"] = sets.New[string]("a-name2", "b-name2")
		condition := &Condition{
			Exact: exact,
		}
		_, err := GetEmptyFilter().FilterObjectList(&list, condition)
	...
	ginkgo.It("TestExactArrayMatch", func() {
		// create condition
		exact := make(map[string]sets.Set[string])
		exact["metadata.annotations.kubeworkz.test.io/app"] = sets.New[string]("a-name1", "a-name2")
		condition := &Condition{
			Exact: exact,
		}
		_, err := GetEmptyFilter().FilterObjectList(&list, condition)
	...
	ginkgo.It("TestArrayFuzzyMatch", func() {
		// create condition
		fuzzy := make(map[string][]string)
		fuzzy["metadata.annotations.kubeworkz.test.io/app"] = []string{"-name2"}
		condition := &Condition{
			Fuzzy: fuzzy,
		}
		_, err := GetEmptyFilter().FilterObjectList(&list, condition)
